...
<template>
    <UPageCard title="Kontakt" class="w-full max-w-lg">
      <UForm
        :schema="schema"
        :state="state"
        ...
        @error="onError"
      >
        <UFormField label="Dit seje navn" name="name" 
        autocomplete="name">
          <UInput v-model="state.name" class="w-full" placeholder="Hej, hva hedder du? .. put her"/>
        </UFormField>

        <UFormField label="Din awesome E-mail" name="email">
          <UInput v-model="state.email" class="w-full" placeholder="volunteerHero@someDomain.lol" 
          autocomplete="email"/>
        </UFormField>
        ...
        >
          ...
          <div class="text-xs text-muted-foreground mt-1">Telefon skal være 8 cifre (valgfrit).</div>
        </UFormField>

        <UFormField label="Emne" name="subject">
          <USelect
        ...
        <UFormField
          ...
          class="w-full"
        >
          <UInput v-model="state.subjectOther" class="w-full" placeholder="Hvad drejer det sig om?" />
        </UFormField>

        <UFormField label="Besked" name="message">
          <UTextarea v-model="state.message" placeholder="Jeg vil snakke om et awesome Frigear projekt..."
        class="w-full" />
        </UFormField>
    <UButton
      type="submit"
