---
title: Projektansøgning
seo:
  title: Projektansøgning - frigear.nu
  description: Information om ansøgningskriterier til Frigear fonden.
---

::u-page-hero
#title
{{ $doc.title }}

#headline
**Information** om ansøgningskriterier for **Frigear projektstøtte 2026**

#links
  :::u-button
  ---
  color: primary
  size: xl
  icon: i-lucide-lightbulb
  trailingIcon: i-lucide-square-arrow-out-up-right
  variant: subtle
  ---
  Pitch dit interne Frigear projekt
  :::

  :::u-button
  ---
  color: secondary
  size: xl
  icon: i-lucide-form
  trailingIcon: i-lucide-arrow-down-from-line
  variant: subtle
  to: /da/funding/#good-to-know
  ---
  Ansøg støtte til eksternt projekt
  :::
  
:::nuxt-img{.mt-12.rounded-xl.hidden.md:block :heigth='400' :width='1200' alt="Frigears frivillige på Roskilde Festival" src="/images/volunteers/fgr/funding-hero-rf-x-50-1200x900.png"}
:::

:::nuxt-img{.mt-4.rounded-xl.block.md:hidden :height='400' :width='640' alt="Frigears frivillige på Roskilde Festival" src="/images/volunteers/fgr/funding-hero-mobile-rf-x-50-1200x900.png"}
:::
::

:::u-page-section{id="good-to-know"}
---
icon: i-lucide-message-circle-question-mark
class: object-cover object-top w-full h-full shadow-2xl ring ring-default rounded-xl
---
#title
Good to know

#description
Foreningen Frigear uddeler årligt en pulje fra forudgående regnskabsårs overskud til awesome, frivilligdrevne projekter i Danmark.
Hvilke projekter der kommer i betragtning, afgøres af foreningens medlemmer på generalforsamlingen (afholdes i 1. kvartal).

Modtagere af **Frigear støttemidler** bliver endeligt afgjort og meldt ud inden **1. juni**. Afgørelsen baseres på indsendte ansøgninger, som skal være bestyrelsen i hænde **senest 10 dage før generalforsamlingen**.
Dato for generalforsamlingen offentliggøres (jf. vedtægter) **senest 3 uger før afholdelse**.
**Husk:** Frigear er udelukkende drevet af frivillige, som bruger deress fritid på at læse jeres ansøgninger igennem — så hjælp os med at hjælpe jer 💜

> **NB:** Frigear puljen går ikke til støtte af enkeltpersoner, institutioner eller foreninger, hvor overskuddet går til enkeltpersoner eller offentlige institutioner eller virkssomheder (her tales ikke om fx husleje eller leje af udstyr i forbindelse med projektet), projekter med lønnede medarbejdere (her tales ikke om fx aflønning af ekstern lyd-tekniker, fragtmand eller lign.), støtte til underskudsdækning, projekter/vedtægter/idégrundlag der går imod Frigears mission, vision, værdier eller vedtægter, eller projekter med politisk eller religiøs agenda.

#features
:::u-page-card
---
icon: i-lucide-calendar-clock
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Hvad er tidslinjen og processserne?

#description
- ✅ **Ansøgninger** skal være bestyrelsen i hænde **senest 10 dage før** generalforsamlingen.  
- ✅ **Dato** for generalforsamlingen offentliggøres (jf. vedtægter) senest 3 uger før afholdelse.
- ✅ **Generalforsamling** afholdes i **1. kvartal**.
- ✅ **Generalforsamlingen henviser** relevante projekter til **bestyrelsen**.
- ✅ **Endeligt svar** gives efter grundig vurdering af ansøgninger og ansøgere af bestyrelsen på **mail senest 1. juni**
:::
:::u-page-card
---
icon: i-lucide-piggy-bank
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Hvor kommer pengene fra?

#description
- ✅ Overskud fra Frigears egne frivilligprojekter og events, gennemført af foreningens medlemmer året igennem.  
- ✅ Evt. privatdonationer, fonde, støttemidler m.v.  
- ✅ Evt. salg af varer, udstyr eller ydelser.  
- ✅ Evt. udlejning af udstyr, lokaler m.m.
- ✅ Evt. samarbejde med andre foreninger, organisationer eller virksomheder.
- ✅ Evt. andre indtægtskilder i tråd med Frigears mission, vision og værdier.
:::

:::u-page-card
---
icon: i-lucide-check-circle
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Hvornår er en ansøgning “med” i vurderingen?

#description
- ✅ Ansøgning indsendt rettidigt.
> Alle ansøgninger indsendt gennem formen inden deadline, -og indeholdende alle relevante oplysninger vil komme i betragtning og blive vurderet af medlemmerne på generalforsamlingen.
- ❌ Ansøgning indsendt efter deadline er som udgangsspunkt afvist.
> Ansøgninger indsendt efter deadline vil ikke komme i betragtning til støtte i det pågældende år medmindre andet aftales med bestyrelsen.
:::

:::u-page-card
---
icon: i-lucide-circle-help
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Hvad har Frigear & Frigear Kulturværksted tidligere støttet?

#description

> Blandt projekter og initiativer Frigear har støttet med midler, frivillige, materialer, konsultering m.v, kan nævnes:
- ✅ **Roskilde Festival** (frivilligprojekter, genbrug og bæredygtige initiativer)
- ✅ **Copenhell** (frivillighed, kultur og Metal musik)
- ✅ **Gutter Island** (sammenhold og kultur med fokus på Garage Rock)
- ✅ **Ungdomshuset** (kulturprojekter og fællesskaber fokuseret omkring unge)
- ✅ **Fællestival** (Festival og social forening med fokus på genbrug, fællesskab og kultur)
- ✅ **Copenhagen B-Roll** (Idræts- Skate eventog kulturarrangement med fokus på rollerskating og kreativitet)
- ✅ **Skuret** (Idræts Skatehal og forening med fokus på børn og unge)
- ✅ **Frigears frivilligprojekter** (fx genbrugsinitiativer, workshops, kulturevents, m.m.)
:::

:::u-page-card
---
icon: i-lucide-sparkles
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Hvad kan Frigear egentlig støtte?

#description
> **Frigear ønsker at støtte**, hvor end det giver mening, ud fra vores kerneværdier som:
-  ✅ **Nonprofit** 
-  ✅ **Frivillighed** 
-  ✅ **Socialt ansvar og samarbejde**
-  ✅ **Bæredygtighed og genbrug**
-  ✅ **Kunst, kultur og kreativitet**
-  ✅ **Inklusion og mangfoldighed**

> **Frirummet er stort**, og vi ønsker ikke at sætte begrænsninger på jeres drømme med specifics, —så lad idéerne flyde 🐼
:::

:::u-page-card
---
icon: i-lucide-circle-x
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Hvad støtter Frigear fx. ikke?

#description
- ❌ **Enkeltpersoner** og **offentlige institutioner** eller **virksomheder**.
> Projekter, eller foreninger hvor overskuddet går til enkeltpersoner eller offentlige institutioner.  
- ❌ **Lønnede medarbejdere**.
> Projekter med lønnede medarbejdere.  
- ❌ **Underskudssdækning**.
> Støtte til projekter eller foreninger med udestående gæld eller udokumenterede regnskaber og ikke transparente økonomiske forhold.  
- ❌ **Ikke imødekommende** ansøgninger.
> Ansøgninger der ikke indeholder alle relevante oplysninger, eller som på anden vis ikke imødekommer de krav og kriterier, som Frigear har opstillet for ansøgninger.
- ❌ **Ikke åbne** foreninger.
> Projekter eller foreninger der ikke er åbne, offentligt tilgængelige, almennyttige og frivilligdrevne.
- ❌ **Ikke forenelige** med Frigears værdier.
> Projekter/foreninger med vedtægter/idégrundlag der går imod Frigears mission, vision, værdier eller vedtægter.  
- ❌ **politik** eller **religion**.
> Projekter eller foreninger med politiske eller religiøse agendaer støttes ikke.
:::

:::u-page-card
---
icon: i-lucide-message-circle
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Hvornår kan vi forvente svar?

#description
- ❌ Hvis ansøgning ikke er kommet i betragtning: 
> Ansøgere hvis ansøgning **IKKE** er kommet i betragtning til potentiel støtte af medlemmerne i år, orienteres umiddelbart efter generalforsamlingen.  
- ✅ Hvis ansøgning er vedtaget til videre behandling af bestyrelsen:
> Ansøgninger, der på generalforsamlingen, af medlemmerne vedtages til videre behandling, overgår til juridisk og økonomisk gennemgang hos bestyrelsen, og modtager endeligt svar inden 1. juni.
:::

:::u-page-card
---
icon: i-lucide-scale
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Hvad hvis vi ikke bruger alle pengene, eller projektet aldrig bli'r til noget?

#description
- ❌ Ubrugte støttemidler tilbageføres. 
> Helt eller delvisst **Ubrugte støttemidler** skal som udgangspunkt **tilbagebetales** til Frigear, så de kan uddeles til andre projekter.
- ✅ Evt. anden anvendelse af støttemidlerne.
> Evt. anden anvendelse i samarbejde med Frigear kan finde sted.
Hvis projektet ikke bli'r til noget, eller hvis I ikke bruger alle pengene, skal I blot give besked til bestyrelsen, så vi kan finde ud af det sammen om evt. anden anvendelse af midlerne i jeres forening.
:::

:::u-page-card
---
icon: i-lucide-receipt-text
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Skal vi aflægge regnskab?

#description
- ❌ **Nej**, som udgangspunkt ikke.

> Vi forbeholder os dog retten til at udbede om **kvitteringer eller dokumentation** på brugen af midlerne i **op til 5 år** -(Jf. gældende lovgivning om regnskabsføring).

- ✅ **Ja** — Såfremt det er relevant.

> I visse tilfælde kan det være nødvendigt at aflægge regnskab over brugen af midlerne, hvis projektet ændrer karakter, eller hvis Frigear har behov for yderligere dokumentation i forhold til regnskabsføring.
:::

:::u-page-card
---
icon: i-lucide-user-lock
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Må projektet/foreningen være lukket for en mindre gruppe?

#description
- ❌ **Nej**.  
Frigear støtter udelukkende åbne, offentligt tilgængelige, almennyttige og frivilligdrevne foreninger og projekter.
:::

:::u-page-card
---
icon: i-lucide-user-check
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Er det en fordel at være medlem af Frigear?

#description
- ❌ **Nej** — alle ansøgninger behandles lige.  
- ✅ **Ja** — medlemskab giver en stemme på generalforsamlingen.

  :::u-button
  ---
  color: primary
  size: lg
  to: /membership
  icon: i-lucide-user-plus
  trailingIcon: i-lucide-square-arrow-out-up-right
  variant: subtle
  width: full
  ---
  Hop hen og bliv medlem
  :::

:::u-page-card
---
icon: i-lucide-heart-crack
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Hvad hvis jeg vil klage over noget?

#description
> Klager kan sendes til bestyrelsen via knappen herunder og/eller kræves taget op på kommende generalforsamling af medlemmer.

::::contact-form-modal{title="Kontakt os"}
---
label: Kontakt bestyrelsen for at klage
initial:
  subject: Klage
---
::::
:::


:::u-page-card
---
icon: i-lucide-megaphone
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Krav om offentlig kreditering

#description
✅ Ja.

Vi er stolte over vores **frivilliges indsats** er med til at støtte jer, og hjælpe med promotion af jeres projekt(er). Derfor forventes det også, at I på samme vis **ønsker at anerkende Frigears støtte** på offentlige medier, SoMe-kanaler m.m. hvor det gi'r mening, samt evt. til event(s) med Frigears logo og evt. links og #hashtags m.v.

**PR materiale** og **brandkit** kan fanges her:

  :::u-button
  ---
  color: secondary
  size: lg
  to: /promo
  icon: i-lucide-qr-code
  trailingIcon: i-lucide-square-arrow-out-up-right
  variant: subtle
  ---
  Promo materiale og brandkit
  :::
:::

:nuxt-img{:height='220' :width='800' alt="Frigear logo / brandkit (indsæt jeres logo her)" src="/images/branding/frigear-logo-kit-800x220.png"}

:::u-page-card
---
icon: i-lucide-clipboard-check
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---

#title
Hvad skal ansøgningen som minimum indeholde?

#description
✅ Lidt om jer.  
✅ Projektets/idéens overordnede formål.  
✅ Antal event- eller projektdeltagere.  
✅ Hvor og hvornår forventes det afholdt/gennemført?  
✅ Er projektet støttet af statsmidler, andre fonde, private eller på anden vis — og i så fald hvem?  
✅ Samlet budget + hvad støttemidlerne specifikt bidrager til.  
✅ Hvor stor en økonomisk andel forventes I selv at bidrage med?  
✅ Forventet overskud — og hvad skal det gå til?  
✅ Organisationens/projektets navn + navne på juridisk ansvarlige.  
✅ CVR på organisationen og/eller CPR på enkeltansvarspersoner.  
✅ Link til hjemmeside og SoMe-kanaler (hvis relevant).  
✅ Bankoplysninger til evt. udbetaling af støtte.
:::

:::u-page-card{orientation="landscape"}
---
icon: i-lucide-mail
class: object-cover object-top w-full h-full shadow-2xl ring ring-default
---
#title
Kontakt Frigear om tvivlsspørgsmål eller sparring på projektidéer

#description
Brug kontaktformularen herunder hvis du har enkeltspørgssmål til ansøgningsprocessen, eller hvis du har en idé til et projekt, som du gerne vil have sparring på, inden du sender en ansøgning:
::::contact-form-modal{title="Kontakt os"}
---
label: Kontakt os om projektstøtte spørgsmål
initial:
  subject: Projektstøtte
---
::::
:::